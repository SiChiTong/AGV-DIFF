...
*	形    参: _ucPort: 端口号(COM1 - COM6)
*	返 回 值: USART_TypeDef*,  USART1, USART2, USART3, UART4, UART5, USART6
*********************************************************************************************************
*/
...
static void UartVarInit(void)
{
	...
	g_tUart1.SendOver = RS485_SendOver;		//RS485_SendBefor				/* 发送完毕后的回调函数 */
...
#if UART6_FIFO_EN == 1			// PC6	TXD6       PC7	RXD6
	/* 第1步： 配置GPIO */
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if (USART_GetITStatus(_pUart->uart, USART_IT_RXNE) != RESET)
	{
		...
		uint8_t ch;
//		biaojiwei = 1;
//		if (_pUart->uart == USART1) //如果是串口1的话，则灯翻转
//		{
//			bsp_LedToggle(4);
//		
//		}
...
*********************************************************************************************************
*/
extern SemaphoreHandle_t xSemaphore;	//二值信号量句柄
extern QueueHandle_t USART1_Queue;	//信息队列句柄
#if UART1_FIFO_EN == 1

//static uint8_t receive_data = 0;
//u16 USART_RX_BUF[111];     //接收缓冲,最大USART_REC_LEN个字节.
...
extern TaskHandle_t xHandle_U2_Navigation = NULL;
#if UART2_FIFO_EN == 1
...
//	bsp_LedToggle(4);
//	/* 发送任务通知 */
//	vTaskNotifyGiveFromISR(xHandle_U2_Navigation, &xHigherPriorityTaskWoken);
...
int fputc(int ch, FILE *f)
{
	...
	USART_SendData(UART6, (uint8_t) ch);
	/* 等待发送结束 */
...
int fgetc(FILE *f)
{
	...
	//return (int)USART_ReceiveData(USART4);
	return (int)USART_ReceiveData(UART6);
#endif
}
